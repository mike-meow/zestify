...
class OnboardingConversation:
    ...
    def get_next_question(self) -> Optional[QuestionResponse]:
        ...
        try:
            ...
            # Print complete raw response if in debug mode
            if self.debug:
                logger.debug("\nComplete OpenRouter Response:")
            ...
            try:
                return QuestionResponse.model_validate(content)
            except Exception as e:
                logger.error(f"Failed to parse QuestionResponse: {str(e)}")
                logger.error(f"Expected structure: {json.dumps(json.loads(QuestionResponse.model_json_schema()), indent=2)}")
                raise
    ...
    def save_profile(self, profile_dir: str = "user_profiles") -> None:
        ...
        user_json_filename = f"{profile_dir}/profile_{self.user_id}.json"
        with open(user_json_filename, 'w') as f:
            json.dump(self.user_profile, f, indent=2)
        logger.info(f"Saved user profile to {user_json_filename}")

        health_json_filename = f"{profile_dir}/health_metrics_{self.user_id}.json"
        with open(health_json_filename, 'w') as f:
            json.dump(self.health_metrics, f, indent=2)
        logger.info(f"Saved health metrics to {health_json_filename}")

    def run(self) -> None:
        """Run the onboarding conversation."""
        logger.info("Starting onboarding conversation")
        print("Welcome to Zestify! Let's create your personalized wellness profile.\n")

        try:
            while self.question_count < self.max_questions:  # Add question limit
                # Get next question from LLM
                question_response = self.get_next_question()
                ...
                print(f"\n{question.question}")
                if question.choices:
                    print(self.format_choices(question.choices))
