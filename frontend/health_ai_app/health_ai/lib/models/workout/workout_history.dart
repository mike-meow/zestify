...
class WorkoutHistory extends BaseModel {
  ...
  int get totalDuration {
    return workouts.fold(
      0, 
      (sum, workout) => sum + workout.durationInSeconds,
    );
  }
  
  /// Get the most common workout type
  WorkoutType? get mostCommonWorkoutType {
    if (workouts.isEmpty) return null;
    
    final typeCounts = <WorkoutType, int>{};
    for (final workout in workouts) {
      typeCounts[workout.type] = (typeCounts[workout.type] ?? 0) + 1;
    }
    
    WorkoutType mostCommon = workouts.first.type;
    int highestCount = 0;
  ...
  Map<String, List<Workout>> get workoutsByMonth {
    ...
    for (final workout in workouts) {
      final monthKey = '${workout.startTime.year}-${workout.startTime.month.toString().padLeft(2, '0')}';
  ...
  Map<WorkoutType, List<Workout>> get workoutsByType {
    final result = <WorkoutType, List<Workout>>{};
    
    for (final workout in workouts) {
      if (!result.containsKey(workout.type)) {
        result[workout.type] = [];
      }
      result[workout.type]!.add(workout);
    }
